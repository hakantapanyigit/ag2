...
import { GalleryPage } from "/snippets/gallery-page.mdx";
import { galleryData } from "/snippets/data/gallery.mdx";
...
To contribute, please open a PR that adds an entry to the `data/gallery.mdx` file in the `snippets` directory. The entry should be an object with the following properties:
