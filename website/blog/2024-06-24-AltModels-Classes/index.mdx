...
Multi-modal support, such as for image/audio/video, is an area of active development. The [Google Gemini](/website/docs/topics/non-openai-models/cloud-gemini) client class can be
...
- **Context length** - as your conversation gets longer, models need to support larger context lengths, be mindful of what the model supports and consider using [Transform Messages](/website/docs/topics/handling_long_contexts/intro_to_transform_messages) to manage context size.
- **Provider parameters** - providers have parameters you can set such as temperature, maximum tokens, top-k, top-p, and safety. See each client class in AutoGen's [API Reference](/website/docs/reference/oai/gemini) or [documentation](/website/docs/topics/non-openai-models/cloud-gemini) for details.
- **Prompts** - prompt engineering is critical in guiding smaller LLMs to do what you need. [ConversableAgent](/website/docs/reference/agentchat/conversable_agent), [GroupChat](/website/docs/reference/agentchat/groupchat), [UserProxyAgent](/website/docs/reference/agentchat/user_proxy_agent), and [AssistantAgent](/website/docs/reference/agentchat/assistant_agent) all have customizable prompt attributes that you can tailor. Here are some prompting tips from [Anthropic](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview)([+Library](https://docs.anthropic.com/en/prompt-library/library)), [Mistral AI](https://docs.mistral.ai/guides/prompting_capabilities/), [Together.AI](https://docs.together.ai/docs/examples), and [Meta](https://llama.meta.com/docs/how-to-guides/prompting/).
...
**NOTE: To integrate this setup into GroupChat, follow the [tutorial](/notebook/agentchat_groupchat) with the same config as above.**
...
For an interesting chess game between Anthropic's Sonnet and Mistral's Mixtral, we've put together a sample notebook that highlights some of the tips and tricks for working with non-OpenAI LLMs. [See the notebook here](/notebook/agentchat_nested_chats_chess_altmodels).
