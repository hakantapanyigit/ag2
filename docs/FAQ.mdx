...
- Autogen can work with any API endpoint which complies with OpenAI-compatible RESTful APIs - e.g. serving local LLM via FastChat or LM Studio. Please check [here](/website/blog/2023-07-14-Local-LLMs) for an example.
- You can supply your own custom model implementation and use it with Autogen. Please check [here](/website/blog/2024-01-26-Custom-Models) for more information.
...
- [`max_consecutive_auto_reply`](/website/docs/reference/agentchat/conversable_agent#max_consecutive_auto_reply) the maximum number of consecutive auto replie (a reply from an agent without human input is considered an auto reply). It plays a role when `human_input_mode` is not "ALWAYS".
- [`max_turns` in `ConversableAgent.initiate_chat`](/website/docs/reference/agentchat/conversable_agent#initiate_chat) limits the number of conversation turns between two conversable agents (without differentiating auto-reply and reply/input from human)
- [`max_round` in GroupChat](/website/docs/reference/agentchat/groupchat#groupchat-objects) specifies the maximum number of rounds in a group chat session.
...
See here /website/docs/reference/agentchat/conversable_agent/#register_reply
...
Refer to /website/docs/reference/agentchat/conversable_agent/#last_message

## How to get each agent message ?

Please refer to /website/docs/reference/agentchat/conversable_agent#chat_messages
