...
configure LLM cache, using either [`DiskCache`](/website/docs/reference/cache/disk_cache#diskcache), [`RedisCache`](/website/docs/reference/cache/redis_cache#rediscache), or Cosmos DB Cache. All agents inside the context manager will use the same cache.
...
By default [`DiskCache`](/website/docs/reference/cache/disk_cache#diskcache) uses `.cache` for storage. To change the cache directory,
set `cache_path_root`:
...
For backward compatibility, [`DiskCache`](/website/docs/reference/cache/disk_cache#diskcache) is on by default with `cache_seed` set to 41.
